...
- 🎞️ 「素材生成」页：上传原剧（超限自动本地压缩）+ 选定配方 → 剪辑方案单（时间线 + ffmpeg 可执行 JSON）
- 🎚️ 可调抽帧 `fps`、模型 ID
...
- **素材生成**（第四个标签页）：上传短剧原始视频（超 50MB 自动本地压缩，或填 URL），选定配方后模型理解剧情并输出剪辑方案单：剪辑时间线（素材时间 ↔ 原剧时间区间 + 字幕/音效/对应手段）+ 执行清单（```json cuts 数组，供 ffmpeg 按 src 裁剪、按 dst 拼接）。方案存 `方案_*.md`，不进提炼；实际剪辑由本地/云主机按 JSON 调 ffmpeg 完成。
